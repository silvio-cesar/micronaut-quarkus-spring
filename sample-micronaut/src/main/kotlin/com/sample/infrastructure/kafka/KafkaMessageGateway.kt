...
class KafkaMessageGateway(
    ...
    @Property(name = "kafka.producers.internal.topic") private val topic: String
) : MessageGateway {
  ...
  private fun produce(operation: String, sample: Sample) {
    ...
    headers.add(RecordHeader(KafkaConstant.OPERATION_HEADER, operation.toByteArray()))
    val dto = sample.toDto()
    producer.send(ProducerRecord(topic, null, Instant.now().toEpochMilli(), dto.id, dto, headers))
  }
}
